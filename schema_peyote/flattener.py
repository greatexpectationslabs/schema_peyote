...
#FIXME: this feels like a really hacky way to achieve this goal.
DISCONNECTED_SUBTREE = 1234.23538902
...
def summarize_json_tree_recursive_iterator(obj, prefix):
    ...
    #FIXME: It's possible to have dicts that should also be treated as iterated objects.
    The cannonical example would be firebase-style JSON objects stored in key-value fashion.
    """
...
def derive_features_from_summary(summary_df):
    ...
    df['children_touch_count'] = df['field'].map(lambda x: df['total_touch_count'][df['parent']==x].sum())
    ...
    df['table_score_z'] =  df.children_touch_count +     6.0*df.data_types.map(lambda x: str(x)=="set(['list'])") +     3.0*df.data_types.map(lambda x: str(x)=="set(['dict'])")
    ...
    def traverse_tree(self, json_obj, prefix, ancestor_prefix, ancestor_id):
        ...
        if isinstance(json_obj, dict):
            ...
            for k, v in json_obj.items():
                child_obj = self.traverse_tree(v, prefix+"."+str(k), new_ancestor_prefix, new_ancestor_id)
                #FIXME: Tre hacky
            ...
            for i, item in enumerate(json_obj):
                child_obj = self.traverse_tree(item, prefix+".*", new_ancestor_prefix, new_ancestor_id)
                #FIXME: Tre hacky
                if child_obj != DISCONNECTED_SUBTREE:
